...
- [BREAKING] **All providers now share an OKF v0.1-compatible `memory/` bundle, while standing role and persona live in `instructions.prepend.md`.** Session-start hooks load `memory/index.md` and `memory/system/definition.md` on startup, clear, and compact, but skip resume. Durable Markdown concepts use YAML frontmatter with a `type`; missing metadata never blocks use and is repaired opportunistically. Normal startup never imports legacy `.seed.md`, pre-composer `CLAUDE.md`, `CLAUDE.local.md`, Claude native auto-memory, or `imported-agent-memory.md`. **Migration:** run `/migrate-memory` before messaging affected groups; the invoking coding harness pauses wakes, stages legacy stores content-blind, quarantines symlinks outside the bundle, organizes every imported fact into the new memory tree, and shows its report for approval before restarting the group. See [docs/provider-migration.md](docs/provider-migration.md).
