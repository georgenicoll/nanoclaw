...
export function renderMemorySection(baseDir = '/workspace/agent'): string {
  ...
  return [
    ...
    '',
    '`memory/` is an Open Knowledge Format (OKF) v0.1 bundle. Every durable',
    'Markdown concept except reserved `index.md` and `log.md` files must start',
    'with YAML frontmatter containing a non-empty `type`. Missing or malformed',
    'metadata never blocks memory use; repair it when you already read or edit',
    'that file. Unknown lowercase kebab-case types are allowed.',
    '',
    '### memory/index.md',
    '',
    index,
